...
public:
    rb_node() : m_red(true), m_value(), m_parent(nullptr), m_left(nullptr), m_right(nullptr) {}
    rb_node(const T &value) : m_red(true), m_value(value), m_parent(nullptr), m_left(nullptr), m_right(nullptr) {}
    ...
    void set_left(rb_node<T> *l) {
        m_left = l;
        if (l) l->m_parent = this;
    }

    void set_right(rb_node<T> *r) {
        m_right = r;
        if (r) r->m_parent = this;
    }
};
...
private:
    ...
    void rotate_right(rb_node<T> *x) {
        ...
        rb_node<T> *p = x->parent();
        if (!p) m_root = y;
        else if (x == p->right()) p->set_right(y);
        else p->set_left(y);
        y->set_right(x);
    }
        ...
        while (node->parent() && node->parent()->is_red()) {
            ...
            if (p == g->left()) {
                ...
                if (u && u->is_red()) {
                    ...
                    if (node == p->right()) { node = p; rotate_left(node); }
                ...
                if (u && u->is_red()) {
                    ...
                    if (node == p->left()) { node = p; rotate_right(node); }
                    rebalance_rotate(p, g, false);
                }
            }
        }
        if (m_root) m_root->set_red(false);
    }

    void fix_remove_case(rb_node<T>* &w, rb_node<T>* p, bool left_side) {
        if (left_side) {
            if (is_black(w->right())) {
                if (w->left()) w->left()->set_red(false);
                w->set_red(true);
                rotate_right(w);
                w = p->right();
            }
            ...
            if (is_black(w->left())) {
                if (w->right()) w->right()->set_red(false);
                w->set_red(true);
                rotate_left(w);
                w = p->left();
            }
    ...
    void fix_remove(rb_node<T> *x) {
        while (x != m_root && is_black(x)) {
            ...
            if (is_black(w->left()) && is_black(w->right())) {
                ...
                if (w) w->set_red(p->is_red());
                p->set_red(false);
                if (left_side) {
                    if (w && w->right()) w->right()->set_red(false);
                    rotate_left(p);
                } else {
                    if (w && w->left()) w->left()->set_red(false);
                    rotate_right(p);
                }
                x = m_root;
            }
        }
        if (x) x->set_red(false);
    }

    rb_node<T> *allocate_node(const T &value) {
        if (m_pool_index >= MAX_NODES) return nullptr;
...
public:
    ...
    rb_node<T> *insert(const T &value) {
        ...
        if (!y) m_root = new_node;
        else if (value < y->value()) y->set_left(new_node);
    ...
    bool remove(const T &value) {
        ...
        if (!z->left()) {
            ...
            transplant(z, y);
            y->set_left(z->left());
            if (y->left()) y->left()->set_parent(y);
            y->set_red(z->is_red());
        }

        if (!y_original_red) fix_remove(x);
        return true;
    }
