...
template <> struct Traits<StringView> {
  static unsigned hash(const StringView &sv) {
    return djb2(sv.data(), sv.size());
  }
...
template <size_t N> struct Traits<fixed_string<N>> {
  static unsigned hash(const fixed_string<N> &s) {
    return djb2(s.c_str(), s.size());
  }
  static void dump(const fixed_string<N> &s) { kprintf("%s", s.c_str()); }
};

template <typename T, size_t N> struct Traits<array<T, N>> {
...
template <typename T> struct Traits<span<T>> {
...
template <typename T, size_t N> struct Traits<static_vector<T, N>> {
...
template <typename T1, typename T2> struct Traits<Pair<T1, T2>> {
...
template <typename T, size_t MaxBits> struct Traits<Bitmap<T, MaxBits>> {
...
template <typename Type, size_t MAX_SIZE> struct Traits<Stack<Type, MAX_SIZE>> {
  static unsigned hash(const Stack<Type, MAX_SIZE> &s) {
    unsigned h = djb2(&s.size(), sizeof(size_t));
    // Hashing stack elements is tricky without exposing internal array or
    // iterator For simplicity, we'll just hash the size for now. A more robust
    // hash would iterate through elements if possible.
    return h;
  }
  static void dump(const Stack<Type, MAX_SIZE> &s) {
    kprintf("Stack(size=%zu, capacity=%zu)", s.size(), s.capacity());
    // Dumping elements would require a peek_at(index) or similar, not currently
    // available. If needed, this would be an enhancement to the Stack class
    // itself.
  }
};

template <typename T> struct Traits<optional<T>> {
...
template <typename T> struct Traits<OwnPtr<T>> {
...
template <typename T> struct Traits<RetainPtr<T>> {
...
template <typename T> struct Traits<rb_node<T>> {
...
template <typename T, unsigned MAX_NODES> struct Traits<rb_tree<T, MAX_NODES>> {
