...
#include <LibFK/Container/Sequence/intrusive_list.h> // Include IntrusiveList
...
public:
    ...
    void remove(T* obj) {
        ...
        // Double-remove guard: unlinked node that isn't the sole head is not in this list.
        if (node.prev == nullptr && node.next == nullptr && m_metadata.m_head != obj)
            return;
    ...
    // Insert obj immediately before position.  If position is nullptr, appends.
    void insert_before(T* position, T* obj) {
        if (!obj) return;
        if (!position) { push_back(obj); return; }

        IntrusiveListNode<T>& node     = obj->m_list_node;
        IntrusiveListNode<T>& pos_node = position->m_list_node;

        node.prev = pos_node.prev;
        node.next = position;

        if (pos_node.prev)
            pos_node.prev->m_list_node.next = obj;
        else
            m_metadata.m_head = obj;
    ...
    // Insert obj in sorted order; cmp(obj, candidate) returns true when obj comes first.
    template <typename Cmp>
    void insert_sorted(T* obj, Cmp&& cmp) {
        if (!obj) return;
        T* pos = m_metadata.m_head;
        while (pos) {
            if (cmp(obj, pos)) break;
            pos = pos->m_list_node.next;
        }
