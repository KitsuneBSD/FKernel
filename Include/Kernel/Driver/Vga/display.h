...
/**
 * @brief Generic linear framebuffer display driver
 * 
 * Uses the framebuffer provided by GOP (UEFI) or Multiboot2 framebuffer tag.
 * Implements basic font rendering for text output.
 */
class display_framebuffer : public display {
private:
...
public:
  static display_framebuffer &the() {
    static display_framebuffer drv;
    return drv;
  }
