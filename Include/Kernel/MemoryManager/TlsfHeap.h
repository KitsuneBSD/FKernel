...
inline void mapping(size_t size, size_t &first_layer, size_t &second_layer) {
  ...
  second_layer =
      (size >> ((first_layer > 5 ? first_layer - 5 : 0))) & (SL_BITS - 1);
}

/**
 * @class TLSFHeap
 * @brief A memory manager implementing the Two-Level Segregate Fit (TLSF)
 * algorithm.
 *
 * This class provides efficient memory allocation and deallocation with
 * constant-time complexity for most operations. It is designed for systems
 * requiring high performance and low fragmentation.
 */
class TLSFHeap {
private:
  /**
   ...
   *
   * This bitmap tracks the availability of blocks in the first-level segregated
   * list.
   */
   ...
   *
   * Each entry in this array corresponds to a first-level block and tracks the
   * availability of blocks in the second-level segregated list.
   */
   ...
   *
   * Each entry corresponds to a specific combination of first-level and
   ...
   * @return A pointer to a suitable free block, or nullptr if none is
   * available.
   */
   ...
   *
   * This constructor sets up the initial heap structure and allocates the first
   * memory region.
   */
  TLSFHeap() {
    ...
    for (size_t i = 0; i < pages_needed; ++i) {
      ...
      if (i == 0)
        m_heap_base = addr;
      MemoryManager::the().map_page(m_heap_base + i * PAGE_SIZE, addr,
...
public:
   ...
   *
   * @param size The size of the memory block to allocate.
   * @param align The alignment requirement for the memory block (default is max
   * alignment).
   ...
   * @return A pointer to the reallocated memory block, or nullptr if
   * reallocation fails.
   */
