...
void VirtualMemoryManager::initialize() {
    ...
    for (auto* node = PhysicalMemoryManager::the().m_memory_ranges.root(); node; /* iter */) {
        ...
        if (range.m_type == MemoryType::Usable) {
            ...
            map_range(range.m_start, range.m_start, range.m_end - range.m_start, PageFlags::Present | PageFlags::CacheDisabled);
        }
        node = node->right();
    }
...
void VirtualMemoryManager::map_page(uintptr_t virt, uintptr_t phys, uint64_t flags) {
    kprintf("Map [ %p -> %p ]\n", phys, virt);
    ...
    if (!pdpt) {
        pdpt = alloc_table();
        m_pml4[pml4_idx] = reinterpret_cast<uint64_t>(pdpt) | PageFlags::Present | PageFlags::Writable;
    }

    uint64_t* pd = reinterpret_cast<uint64_t*>(pdpt[pdpt_idx] & ~0xFFF);
    if (!pd) {
        pd = alloc_table();
        pdpt[pdpt_idx] = reinterpret_cast<uint64_t>(pd) | PageFlags::Present | PageFlags::Writable;
    }

    uint64_t* pt = reinterpret_cast<uint64_t*>(pd[pd_idx] & ~0xFFF);
    if (!pt) {
        pt = alloc_table();
        pd[pd_idx] = reinterpret_cast<uint64_t>(pt) | PageFlags::Present | PageFlags::Writable;
    }

    pt[pt_idx] = phys | flags;
}

void VirtualMemoryManager::map_range(uintptr_t virt_start, uintptr_t phys_start, size_t size, uint64_t flags) {
...
uintptr_t VirtualMemoryManager::virt_to_phys(uintptr_t virt) {
    ...
    if (!(entry & PageFlags::Present)) return 0;
