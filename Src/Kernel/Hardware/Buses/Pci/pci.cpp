...
void PciManager::handle_hotplug_event(uint8_t bus, uint8_t device, uint8_t function) {
    ...
    if (exists && known_index == m_devices.size()) {
        ...
        if (m_pci_node) {
            m_pci_node->push_event({fkernel::PCIEventType::Insertion, bus, device, function, vendor, dev_id});
        }
        ...
        // Notify userspace node
        if (m_pci_node) {
            m_pci_node->push_event({fkernel::PCIEventType::Removal, bus, device, function, removed_device.vendor_id(), removed_device.device_id()});
        }
