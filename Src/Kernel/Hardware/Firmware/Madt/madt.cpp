#include <LibFK/Core/assertions.h>
#include <LibFK/Algorithms/Logging/log.h>

#include <Kernel/Hardware/Firmware/Acpi/acpi.h>
#include <Kernel/Hardware/Firmware/Madt/madt.h>
#include <Kernel/Hardware/Firmware/Madt/madt_entries.h>
#include <Kernel/Hardware/Firmware/Madt/madt_lapic.h>
#include <Kernel/Hardware/Firmware/Madt/madt_ioapic.h>
#include <Kernel/Hardware/Firmware/Madt/madt_interrupt_source_override.h>
#include <Kernel/Hardware/Firmware/Madt/madt_lapic_override.h>

void ACPIManager::initialize_madt() {
  m_madt = (Madt *)find_table("APIC");
  assert(m_madt && "MADT table not found - APIC infrastructure unavailable!");

  fk::algorithms::klog("MADT", "MADT found at %p", m_madt);
  fk::algorithms::klog("MADT", "MADT signature: %.4s", &m_madt->header.signature);
  fk::algorithms::klog("MADT", "MADT length: %u bytes", m_madt->header.length);
  fk::algorithms::klog("MADT", "Local APIC address: %p", (void *)(uintptr_t)m_madt->lapic_address);
  fk::algorithms::klog("MADT", "MADT flags: 0x%x", m_madt->flags);
  
  process_madt_entries();
}

void ACPIManager::process_madt_entries() {
  assert(m_madt && "process_madt_entries called without valid MADT!");

  uint8_t *entries_start = m_madt->entries;
  uint8_t *entries_end = (uint8_t *)m_madt + m_madt->header.length;
  uint32_t entry_count = 0;

  fk::algorithms::klog("MADT", "Processing MADT entries...");

  for (uint8_t *p = entries_start; p < entries_end; ) {
    MadtEntry *entry = (MadtEntry *)p;
    
    if (entry->length < 2) {
        fk::algorithms::kwarn("MADT", "Invalid entry length %u at offset %tu! Aborting.", entry->length, p - entries_start);
        break;
    }
    if (p + entry->length > entries_end) {
        fk::algorithms::kwarn("MADT", "Entry length %u at offset %tu exceeds table bounds! Aborting.", entry->length, p - entries_start);
        break;
    }

    switch (entry->type) {
      case 0: { // Processor Local APIC
        auto *lapic = (MADT_LAPIC *)entry;
        bool enabled = (lapic->flags & 1);
        bool online_capable = (lapic->flags & 2);
        
        if (enabled) {
            if (m_cpu_count < MAX_CPU_APIC_IDS) {
                m_cpu_apic_ids[m_cpu_count] = lapic->apic_id;
            }
            m_cpu_count++;
            fk::algorithms::klog("MADT", "  Entry %u: Processor Local APIC (ACPI ID: %u, APIC ID: %u) - [ENABLED]",
                                 entry_count, lapic->acpi_processor_id, lapic->apic_id);
        } else if (online_capable) {
            fk::algorithms::klog("MADT", "  Entry %u: Processor Local APIC (ACPI ID: %u, APIC ID: %u) - [DISABLED / HOTPLUG CAPABLE]",
                                 entry_count, lapic->acpi_processor_id, lapic->apic_id);
        } else {
            fk::algorithms::klog("MADT", "  Entry %u: Processor Local APIC (ACPI ID: %u, APIC ID: %u) - [DISABLED]",
                                 entry_count, lapic->acpi_processor_id, lapic->apic_id);
        }
        break;
      }
      case 1: { // I/O APIC
        auto *ioapic = (MADT_IOAPIC *)entry;
        fk::algorithms::klog("MADT", "  Entry %u: I/O APIC (ID: %u, Address: %p, GSI Base: %u)",
                             entry_count, ioapic->ioapic_id, (void *)(uintptr_t)ioapic->address, ioapic->gsi_base);
        if (m_ioapic_count < MAX_IOAPICS) {
            m_ioapics[m_ioapic_count].id = ioapic->ioapic_id;
            m_ioapics[m_ioapic_count].address = ioapic->address;
            m_ioapics[m_ioapic_count].gsi_base = ioapic->gsi_base;
            m_ioapic_count++;
        }
        if (ioapic->address != 0 && ioapic->gsi_base == 0)
          set_ioapic_address(static_cast<uintptr_t>(ioapic->address));
        break;
      }
      case 2: { // Interrupt Source Override
        auto *override = (MADT_InterruptSourceOverride *)entry;
        fk::algorithms::klog("MADT", "  Entry %u: Interrupt Source Override (Bus: %u, IRQ: %u -> GSI: %u, Flags: 0x%x)",
                             entry_count, override->bus_source, override->irq_source, override->gsi, override->flags);
        if (m_iso_count < MAX_ISOS) {
            m_isos[m_iso_count] = *override;
            m_iso_count++;
        }
        break;
      }
      case 3: // NMI Source
        fk::algorithms::klog("MADT", "  Entry %u: NMI Source (type %u, length %u)",
                             entry_count, entry->type, entry->length);
        break;
      case 4: // Local APIC NMI
        fk::algorithms::klog("MADT", "  Entry %u: Local APIC NMI (type %u, length %u)",
                             entry_count, entry->type, entry->length);
        break;
      case 5: { // Local APIC Address Override
        auto *lapic_override = (MADT_LAPIC_OVERRIDE *)entry;
        fk::algorithms::klog("MADT", "  Entry %u: Local APIC Address Override (Address: %p)",
                             entry_count, (void *)lapic_override->lapic_address);
        break;
      }
      default:
        fk::algorithms::klog("MADT", "  Entry %u: Unknown type %u (length %u)",
                             entry_count, entry->type, entry->length);
        break;
    }

    entry_count++;
    p += entry->length;
  }

  fk::algorithms::klog("MADT", "Total MADT entries processed: %u", entry_count);
  fk::algorithms::klog("MADT", "Detected %u CPUs, %u IO-APICs, %u ISOs",
                       m_cpu_count, m_ioapic_count, m_iso_count);
}
