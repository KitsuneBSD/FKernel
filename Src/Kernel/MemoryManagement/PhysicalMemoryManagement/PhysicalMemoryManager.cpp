...
{
    total_page_size = 4096; // Definição padrão, 4 KiB
    ...
    LibC::uint64_t total_memory_bytes = highest_addr - lowest_addr;
    pmm_total_pages = total_memory_bytes / total_page_size;
    ...
    for (LibC::uint64_t i = 0; i < pmm_total_pages; ++i) {
        if (!get_bit(i)) { // Página livre (bit 0)
            if (current_len == 0)
                current_start = i;
            ++current_len;
    ...
    Logf(LogLevel::INFO,
        ...
        total_memory_bytes / FK::KiB, total_memory_bytes / FK::MiB,
        (free_pages * total_page_size) / FK::KiB, (free_pages * total_page_size) / FK::MiB);
...
void PhysicalMemoryManager::add_free_block(LibC::uint64_t start_page, LibC::uint64_t page_count) noexcept
{
    ...
    for (LibC::size_t i = 0; i < free_block_count;) {
        if (free_blocks[i].overlaps(new_block) || free_blocks[i].adjacent_to(new_block)) {
            new_block.merge_with(free_blocks[i]);

            for (LibC::size_t k = i; k + 1 < free_block_count; ++k)
...
void PhysicalMemoryManager::free_page(LibC::uintptr_t phys_addr) noexcept
{
    ...
    if ((phys_addr - pmm_base_addr) % total_page_size != 0)
        return;
