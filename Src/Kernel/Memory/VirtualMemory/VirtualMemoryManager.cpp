...
uintptr_t VirtualMemoryManager::translate(uintptr_t virt) {
    ...
    if (!(m_pml4->entries[pml4_idx] & static_cast<uint64_t>(PageFlags::Present))) return 0;
    PageTable* pdpt = reinterpret_cast<PageTable*>(m_pml4->entries[pml4_idx] & 0x000FFFFFFFFFF000);

    if (!(pdpt->entries[pdpt_idx] & static_cast<uint64_t>(PageFlags::Present))) return 0;
    PageTable* pd = reinterpret_cast<PageTable*>(pdpt->entries[pdpt_idx] & 0x000FFFFFFFFFF000);

    if (!(pd->entries[pd_idx] & static_cast<uint64_t>(PageFlags::Present))) return 0;
    PageTable* pt = reinterpret_cast<PageTable*>(pd->entries[pd_idx] & 0x000FFFFFFFFFF000);

    if (!(pt->entries[pt_idx] & static_cast<uint64_t>(PageFlags::Present))) return 0;
    return (pt->entries[pt_idx] & 0x000FFFFFFFFFF000) + (virt & 0xFFF);
}
