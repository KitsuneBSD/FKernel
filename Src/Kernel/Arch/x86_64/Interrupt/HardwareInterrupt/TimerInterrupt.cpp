...
void TimerManager::set_timer(Timer *timer) {
  m_timer = timer;
}
...
void TimerManager::set_memory_manager(bool has_memory_manager) {
  ...
  if (!old_has_memory_manager && m_has_memory_manager) {
    fk::algorithms::klog("TIMER", "Memory manager enabled, re-evaluating timer controller.");
    // Re-select and configure the timer with the current frequency (assuming a default or stored freq)
    // For now, we'll use a placeholder frequency. A better approach would be to store the frequency.
    select_and_configure_timer(100); // Re-initialize with a default frequency
    TickManager::the().set_frequency(100); // Update TickManager with the default frequency
  }
}
