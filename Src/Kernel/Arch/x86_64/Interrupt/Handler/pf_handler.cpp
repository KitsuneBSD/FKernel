...
void page_fault_handler([[maybe_unused]] uint8_t vector,
    ...
    kexception("PAGE FAULT", "RIP=%lx CS=%lx RFLAGS=%lx", frame->rip, frame->cs, frame->rflags);
    kexception("PAGE FAULT", "RSP=%lx SS=%lx", frame->rsp, frame->ss);
    kexception("PAGE FAULT", "Fault addr (CR2)=%p, error=%lx", cr2, frame->error_code);

    bool non_present = !(frame->error_code & 1);

    if (non_present) {
        kexception("PAGE FAULT", "  -> Non-present page");

        // Identity map do APIC se necessário
        if (cr2 >= 0xFEE00000 && cr2 < 0xFEE20000) {
            uint64_t page = cr2 & ~0xFFFULL;
            VirtualMemoryManager::the().map_page(
                page, page,
                PageFlags::Present | PageFlags::Writable | PageFlags::CacheDisabled
            );
            klog("PAGE FAULT", "Mapped APIC page at 0x%lx", page);
            return;
        }
    }
