...
static int block_device_read(VNode* vnode, FileDescriptor* fd, void* buffer, size_t size, size_t offset) {
    BlockDevice* dev = get_block_device(vnode);
    if (!dev) return -1;
    return dev->read(vnode, fd, buffer, size, offset);
}

static int block_device_write(VNode* vnode, FileDescriptor* fd, const void* buffer, size_t size, size_t offset) {
    BlockDevice* dev = get_block_device(vnode);
    if (!dev) return -1;
