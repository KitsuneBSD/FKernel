...
int GptPartitionStrategy::parse(const void *sector512, PartitionEntry *out,
  ...
  if (hdr->header_size < 92 || hdr->header_size > 512) {
    fk::algorithms::kwarn("GPT", "GPT Header size invalid: %u",
                          hdr->header_size);
    return 0;
  }
  ...
  *reinterpret_cast<uint32_t *>(header_buffer + 16) = 0;
  ...
  uint32_t calculated_header_crc32 =
      fk::algorithms::crc32(header_buffer, header_size);

  if (calculated_header_crc32 != original_header_crc32) {
    fk::algorithms::kwarn("GPT",
                          "GPT Header CRC32 mismatch: expected %08x, got %08x",
                          original_header_crc32, calculated_header_crc32);
    return 0; // CRC mismatch, invalid header
  }
  if (hdr->signature != 0x5452415020494645ULL) {
    fk::algorithms::kwarn("GPT", "Invalid signature");
    return 0;
  }

  if (hdr->partition_entry_size != sizeof(GptEntry)) {
    fk::algorithms::kwarn("GPT",
                          "Unexpected GPT entry size: expected %zu, got %u",
  ...
  uint32_t calculated_pe_crc32 = fk::algorithms::crc32(
      partition_entry_array_start, partition_entry_array_size);
