...
#include <Kernel/Posix/errno.h> // Missing include
...
int PartitionBlockDevice::read(VNode *vnode, FileDescriptor *fd, void *buffer, size_t size, size_t offset)
{
    ...
    // Read from the underlying block device, adjusting the offset for the partition
    return pinfo->device->read(vnode, fd, buffer, size, pinfo->lba_first * 512 + offset);
}
...
int PartitionBlockDevice::write(VNode *vnode, FileDescriptor *fd, const void *buffer, size_t size, size_t offset)
{
    ...
    // Write to the underlying block device, adjusting the offset for the partition
    return pinfo->device->write(vnode, fd, buffer, size, pinfo->lba_first * 512 + offset);
}
