...
fk::core::Result<void, fk::core::Error> DriverManager::register_device(fk::RefPtr<Node> device) {
  ...
  if (res.is_error()) {
    fk::algorithms::kwarn("DRIVER_MANAGER", "Failed to expose '%s' in DevFs: %d",
                          device->name().c_str(), (int)res.error());
  } else {
    fk::algorithms::klog("DRIVER_MANAGER", "Device '%s' exposed in DevFs successfully",
                         device->name().c_str());
  }
...
void DriverManager::unregister_device(fk::RefPtr<Node> device) {
  for (size_t i = 0; i < m_devices.size(); ++i) {
    if (m_devices[i] == device) {
      fk::algorithms::klog("DRIVER_MANAGER", "Unregistering device: '%s'", device->name().c_str());
  ...
  fk::algorithms::kwarn("DRIVER_MANAGER", "unregister_device: device not found in registry");
}

void DriverManager::probe_all() {
  fk::algorithms::klog("DRIVER_MANAGER", "Probing all %d registered drivers...", m_drivers.size());
  for (auto& driver : m_drivers) {
    fk::algorithms::klog("DRIVER_MANAGER", "Probing driver: '%s'...", driver->name());
    driver->probe();
  }
  fk::algorithms::klog("DRIVER_MANAGER", "Probe sequence completed.");
}
