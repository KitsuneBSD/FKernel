...
fk::core::Result<void, fk::core::Error> DriverManager::register_driver(fk::memory::OwnPtr<Driver> driver) {
    if (!driver) return fk::core::Error::InvalidParameter;
...
fk::core::Result<void, fk::core::Error> DriverManager::register_device(fk::RefPtr<Node> device) {
    ...
    fk::algorithms::klog("DRIVER MANAGER", "Registering device: '%s' (Type: %s)", 
                         device->name().c_str(), 
    ...
    if (res.is_error()) {
        ...
        fk::algorithms::klog("DRIVER MANAGER", "Device '%s' exposed in DevFs successfully", device->name().c_str());
    }
