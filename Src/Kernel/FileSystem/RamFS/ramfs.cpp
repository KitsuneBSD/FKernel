...
int RamFS::ramfs_read(VNode *vnode, FileDescriptor *fd, void *buffer, size_t size, size_t offset)
{
    ...
    {
        kwarn("RamFS", "Read failed: vnode '%s' is not a regular file", vnode->m_name.c_str());
        return -1;
    }

    if (!vnode->fs_private)
    {
        kwarn("RamFS", "Read failed: vnode '%s' has no fs_private data", vnode->m_name.c_str());
        return -1;
    }
...
int RamFS::ramfs_write(VNode *vnode, FileDescriptor *fd, const void *buffer, size_t size, size_t offset)
{
    ...
    if (vnode->type != VNodeType::Regular)
    {
        kwarn("RamFS", "Write failed: vnode '%s' is not a regular file", vnode->m_name.c_str());
        return -1;
    }

    if (!vnode->fs_private)
    {
        kwarn("RamFS", "Write failed: vnode '%s' has no fs_private data", vnode->m_name.c_str());
        return -1;
    }
    ...
    if (offset >= MAX_SIZE)
    {
        kwarn("RamFS", "Write failed: offset %zu exceeds maximum file size %zu in '%s'", offset, MAX_SIZE, vnode->m_name.c_str());
        return -1;
    }
...
int RamFS::ramfs_unlink(VNode *vnode, FileDescriptor *fd, const char *name)
{
    ...
    for (size_t i = 0; i < vnode->dir_entries.size(); ++i)
    {
        if (strcmp(vnode->dir_entries[i].m_name.c_str(), name) == 0)
        {
            ...
            klog("RamFS", "Unlinked '%s' from directory '%s'", name, vnode->m_name.c_str());
            return 0;
        }
    }
