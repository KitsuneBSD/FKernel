...
void VFS::init()
{
    ...
    m_root = new VFSNode("/", FileType::Directory, perms);
...
VFSNode *VFS::create_node(const char *name, FileType type, FilePermissions perms)
{
    auto node = new VFSNode(name, type, perms);
    return node;
}

VFSNode *VFS::lookup_child(VFSNode *parent, const char *name)
{
...
{
    if (!node || node->type != FileType::Regular)
        return -1;
    if (!node->ops)
        return -1;
