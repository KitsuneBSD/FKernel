...
extern "C" uint64_t sys_clock_nanosleep([[maybe_unused]] uint64_t clk_id,
                                         [[maybe_unused]] uint64_t flags,
                                         uint64_t req_ptr, uint64_t rem_ptr,
                                         uint64_t, uint64_t,
                                         [[maybe_unused]] PtRegs* regs) {
  auto* req = reinterpret_cast<const timespec*>(req_ptr);
  ...
  if (rem_ptr) {
    auto* rem = reinterpret_cast<timespec*>(rem_ptr);
    rem->tv_sec = 0;
    rem->tv_nsec = 0;
  }
  return 0;
