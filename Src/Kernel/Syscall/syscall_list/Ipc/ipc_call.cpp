...
extern "C" uint64_t sys_ipc_call(uint64_t endpoint_id, uint64_t info_raw,
  ...
  auto result = endpoint->call(info, message);
  if (result.is_error())
    return (uint64_t)-1;

  const IpcMessage &reply = task->ipc().pending_message;
  if (regs) {
    regs->rdx = reply.word(0);
    regs->r10 = reply.word(1);
    regs->r8 = reply.word(2);
    regs->r9 = reply.word(3);
  }
