...
// sys_futimesat(dirfd, path, times[2]) → 0 or -errno
// times == NULL means set both to now. Legacy BSD interface: timeval, no
// UTIME_* sentinels.
extern "C" uint64_t sys_futimesat(uint64_t dirfd_u, uint64_t path_ptr, uint64_t times_ptr,
  ...
  if (times_ptr != 0) {
    ...
    for (int i = 0; i < 2; ++i) {
      if (user_times[i].tv_usec < 0 || user_times[i].tv_usec >= 1000000)
        return return_error(fk::core::Error::InvalidParameter);
    }
    have_times = true;
  }
  ...
  timespec mtime{0, 0};
  if (have_times) {
    atime.tv_sec = user_times[0].tv_sec;
    atime.tv_nsec = user_times[0].tv_usec * 1000;
    mtime.tv_sec = user_times[1].tv_sec;
    mtime.tv_nsec = user_times[1].tv_usec * 1000;
  } else {
