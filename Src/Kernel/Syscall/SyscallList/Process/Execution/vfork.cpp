...
extern "C" uint64_t sys_vfork([[maybe_unused]] uint64_t arg1, [[maybe_unused]] uint64_t arg2,
  ...
  for (size_t i = 0; i < parent->resources.files.descriptors.size(); ++i) {
    ...
    TRY_OR_FATAL(child->resources.files.descriptors.push_back(desc));
    ...
    TRY_OR_FATAL(child->resources.files.cap_handles.push_back(child_handle));
  }
  ...
  fk::memory::copy(child_stack_mem,
  ...
  for (size_t i = 0; i < parent->resources.memory.regions.list.size(); ++i) {
    TRY_OR_FATAL(child->resources.memory.regions.list.push_back(parent->resources.memory.regions.list[i]));
  }
