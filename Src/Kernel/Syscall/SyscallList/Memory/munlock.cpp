...
// sys_munlock(addr, len) → 0 or -errno
// FKernel has no swap, so unlocking is a no-op. Validate the range like mlock.
extern "C" uint64_t sys_munlock(uint64_t addr_u, uint64_t len_u, uint64_t, uint64_t, uint64_t,
