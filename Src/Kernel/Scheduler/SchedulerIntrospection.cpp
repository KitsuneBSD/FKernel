...
Task* SchedulerManager::find_task(fk::ProcessId id) {
  for (uint32_t i = 0; i < m_processor_count; ++i) {
    if (m_processors[i].current_task && m_processors[i].current_task->control.identity.id == id) return m_processors[i].current_task;
    if (m_processors[i].idle_task && m_processors[i].idle_task->control.identity.id == id) return m_processors[i].idle_task;
    {
