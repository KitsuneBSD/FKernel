...
if is_mb or is_mb2 then
    ...
    PrintMessage(true, "Hint: Ensure `.multiboot_header` is present within the first 32KiB of the binary and is part of a loadable PT_LOAD segment.")
    PrintMessage(true, "Suggestion: run `readelf -l` and inspect the program headers and `readelf -S` for the .multiboot_header placement.")
