...
if OSInteract.CopyFile(grub_cfg, grub_dir .. "/grub.cfg") then
	PrintMessage(false, "Copied grub config")
else
	PrintMessage(true, "Failed to copy grub config")
end

if OSInteract.CopyFile(kernel_bin, "build/mockos/boot/FKernel.bin") then
...
if OSInteract.FileExists(fkernel_mockos) then
...
if not OSInteract.FileExists(hda_path) then
